...
// ===== COBALT TOKEN =====
// Cache in-memory (SW có thể restart, token sẽ được fetch lại tự động)
let cobaltTokenCache = null; // { token, exp }

async function getCobaltToken() {
  const now = Date.now();
  if (cobaltTokenCache && now < cobaltTokenCache.exp - 30_000) {
    return cobaltTokenCache.token;
  }
  const res = await fetch('https://api.cobalt.tools/session', {
    method: 'POST',
    headers: { 'Accept': 'application/json' },
  });
  if (!res.ok) throw new Error(`cobalt session error: ${res.status}`);
  const data = await res.json();
  cobaltTokenCache = { token: data.token, exp: (data.exp || 0) * 1000 };
  return data.token;
}

async function cobaltRequest(message, token) {
  const res = await fetch('https://api.cobalt.tools/', {
    ...
    body: JSON.stringify({
      url:           message.url,
      videoQuality:  message.quality  || 'max',
      downloadMode:  message.mode     || 'auto',
      audioFormat:   'mp3',
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // Gọi tikwm.com API để lấy link download
  if (message.type === 'TIKWM_FETCH') {
    fetch('https://www.tikwm.com/api/', {
      ...
      body: new URLSearchParams({ url: message.url, hd: '1' }),
    })
      .then((r) => r.text())
      .then((text) => {
        let json;
        try { json = JSON.parse(text); } catch (_) {
          sendResponse({ ok: false, error: 'tikwm không phản hồi JSON' });
          return;
        }
        if (json.code === 0) {
          sendResponse({ ok: true, data: json.data });
        } else {
          sendResponse({ ok: false, error: json.msg || 'API error' });
        }
      })
  ...
  // YouTube innertube API — iOS client trả về URL stream không cần decipher
  if (message.type === 'YT_FETCH_STREAMS') {
    (async () => {
      try {
        ...
        // Đọc text trước, parse JSON sau — tránh crash khi YouTube trả HTML
        const text = await res.text();
        let data;
        try {
          data = JSON.parse(text);
        } catch (_) {
          sendResponse({ ok: false, error: `HTTP ${res.status} — YouTube không trả JSON (${text.slice(0, 80)})` });
          return;
        }
        ...
        // Best audio stream
        const audioList = adaptiveFormats
          .filter((f) => f.url && f.mimeType?.startsWith('audio/'))
          .sort((a, b) => (b.bitrate || 0) - (a.bitrate || 0));
        const audio = audioList[0] ? {
          url:      audioList[0].url,
          mimeType: audioList[0].mimeType,
          itag:     audioList[0].itag,
          size:     audioList[0].contentLength ? parseInt(audioList[0].contentLength) : null,
          bitrate:  audioList[0].bitrate || 0,
        } : null;

        // Metadata từ videoDetails
        ...
        sendResponse({ ok: true, info, combined, audio });
      } catch (e) {
  ...
  if (message.type === 'COBALT_FETCH') {
    (async () => {
      try {
        const token = await getCobaltToken();
        const data  = await cobaltRequest(message, token);

        // Token hết hạn → xoá cache, thử lại 1 lần
        if (data?.error?.code === 'error.api.auth.jwt.expired' ||
            data?.error?.code === 'error.api.auth.jwt.missing') {
          cobaltTokenCache = null;
          const fresh = await getCobaltToken();
          sendResponse({ ok: true, data: await cobaltRequest(message, fresh) });
        } else {
